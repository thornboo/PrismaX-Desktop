...
<kbDir>/
  ...
  blobs/                  # 原始文件/图片等（当前：sha256 内容寻址 + ref_count 去重）
    sha256/ab/cd/<sha256>
  index/
...
- `file_fingerprints`（实现细节）：`source_path + size + mtime -> sha256`，用于增量导入避免重复 hash
- `blobs`（实现细节）：`sha256 -> rel_path + size + ref_count`，用于内容寻址去重与 GC
...
- 引擎：LanceDB（本地目录存储，适合嵌入式桌面端分发）
- 数据：按 KB 目录存放于 `index/vector/lancedb/`（LanceDB 数据集目录）
- 任务：`build_vectors`（批量 embedding → upsert 到 LanceDB → 记录已索引 chunk）
- Meta DB：
  - `vector_config`：记录当前索引的 `providerId/model/dimension/updatedAt`
  - `chunk_vectors`：记录 `chunk_id` 在某个 `config_hash` 下已索引（用于断点续跑/增量构建）
- Embedding：复用现有云端 provider（OpenAI-compatible `/v1/embeddings`），API Key **不落盘**，仅在任务执行/检索时通过 IPC 临时传入

约束（产品/工程一致性）：

- 当 `providerId/model/dimension` 发生变化时，必须先执行“重建向量索引”（删除派生数据），再重新构建；避免混用不同 embedding 配置导致召回不可解释。
- 向量索引属于派生数据：允许删除、允许重建，不参与“schema migration 的长耗时回填”。
